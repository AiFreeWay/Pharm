...
public class RecordMapper {
    ...
    public static Record mapRecord(Row row) throws Exception {
        ...
        String series = escapeQuote(CELL_FORMATTER.formatCellValue(row.getCell(1)));
        ...
        return new Record(series.hashCode(), series, title, provider, certificate, date, description);
    }

    public static void fillRow(Record record, Row row) throws Exception {
        row.createCell(0).setCellValue(record.getTitle());
        row.createCell(1).setCellValue(record.getSeries());
